...
import { Paciente, Prescripcion, HorarioTipo, ViaAdministracion, NuevaPrescripcionAlergiaData } from '../types';
import { AlertTriangle, User, Pill, Activity, Clock, History, Link, ArrowRight, ChevronLeft, Check } from 'lucide-react';
...
const PrescripcionAlergiaPage: React.FC = () => {
  const navigate = useNavigate();
  const [pacientes, setPacientes] = useState<Paciente[]>([]);
  const [prescripcionesAnteriores, setPrescripcionesAnteriores] = useState<Prescripcion[]>([]);
  ...
  const [form, setForm] = useState<NuevaPrescripcionAlergiaData>({
    medicamento: '',
    horario: 8,
    tiempoTratamiento: 7,
    dosis: '',
  ...
  useEffect(() => {
    if (form.pacienteId) {
      prescripcionesAPI
        .getActivas(form.pacienteId)
        .then(setPrescripcionesAnteriores)
        .catch(console.error);
    } else {
      setPrescripcionesAnteriores([]);
    }
  }, [form.pacienteId]);
  ...
  const handleChange = (field: keyof NuevaPrescripcionAlergiaData, value: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await prescripcionesAPI.crearPorAlergia(form);
      toast.success('Nueva prescripción vinculada con éxito');
      navigate('/dashboard');
    } catch (err: any) {
      setError(err.response?.data?.error || 'Error al crear prescripción');
      toast.error('No se pudo completar el cambio de prescripción');
    } finally {
      setLoading(false);
    }
  };
  ...
  const selectedAnterior = prescripcionesAnteriores.find((p) => p._id === form.prescripcionAnteriorId);
    ...
    <div className="max-w-3xl mx-auto space-y-8 animate-fade-in pb-12">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 flex items-center gap-2">
            ...
            Cambio por Contraindicación
          </h1>
          <p className="text-gray-500 font-medium">Reemplazo de prescripción activa por alergia o efectos adversos</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card className="p-8 space-y-8 shadow-2xl border-white/50 backdrop-blur-xl bg-white/80">
          ...
          {/* Prescripción a Reemplazar */}
          <div className="space-y-4">
             <div className="space-y-1.5">
                <label className="block text-sm font-semibold text-gray-700 ml-1">Prescripción a Reemplazar</label>
                <select
                  value={form.prescripcionAnteriorId}
                  onChange={(e) => handleChange('prescripcionAnteriorId', e.target.value)}
                  className="input-field"
                  required
                  disabled={!form.pacienteId}
                >
                  <option value="">Seleccionar prescripción activa...</option>
                  {prescripcionesAnteriores.map((p) => (
                    <option key={p._id} value={p._id}>
                      {p.medicamento} - {p.dosis} (Cada {p.horario}h)
                    </option>
                  ))}
                </select>
             </div>
             ...
             <AnimatePresence mode="wait">
               {selectedAnterior && (
                 <motion.div
                   key={selectedAnterior._id}
                   initial={{ opacity: 0, x: -20 }}
                   animate={{ opacity: 1, x: 0 }}
                   exit={{ opacity: 0, x: 20 }}
                 >
                    <Card variant="outline" className="bg-gray-50/50 p-4 border-dashed relative overflow-hidden group">
                       <div className="absolute top-0 right-0 p-2 bg-red-100 text-red-600 rounded-bl-xl text-[10px] font-black uppercase tracking-wider">
                          Se desactivará
                       </div>
                       <div className="flex items-center gap-4">
                          ...
                          <div>
                             <h4 className="font-bold text-gray-900">{selectedAnterior.medicamento}</h4>
                             <p className="text-xs text-gray-500">
                                {selectedAnterior.dosis} • Cada {selectedAnterior.horario}h • {selectedAnterior.viaAdministracion}
                             </p>
                          </div>
                       </div>
                    </Card>
          ...
          {/* Motivo */}
          <div className="space-y-1.5">
             <label className="block text-sm font-semibold text-gray-700 ml-1">Motivo del Cambio</label>
             <textarea
                value={form.motivoCambio}
                onChange={(e) => handleChange('motivoCambio', e.target.value)}
                className="input-field min-h-[100px] py-3"
                placeholder="Ej: El paciente presentó erupciones cutáneas tras la primera dosis (Considerado Alergia)..."
                required
             />
             <div className="flex items-center gap-2 mt-2 px-1">
                <div className="p-1 bg-medical-100 rounded-full">
                   <Activity className="w-3 h-3 text-medical-600" />
                </div>
                <p className="text-[11px] text-gray-500 font-medium">
                   Al mencionar <span className="text-medical-600 font-bold italic">"alergia"</span>, se registrará automáticamente en la ficha del paciente.
                </p>
             </div>
          </div>
          ...
          <div className="flex items-center justify-center py-2">
             ...
             <div className="px-4 py-1 bg-gray-100 rounded-full text-[10px] font-black text-gray-400 uppercase tracking-widest">
                Nueva Alternativa
             </div>
             <div className="h-px bg-gray-100 flex-grow" />
          </div>
          ...
          <div className="flex gap-4 pt-4">
             ...
             <Button 
                type="submit" 
                className="flex-1 shadow-xl shadow-amber-200 !bg-amber-500 hover:!bg-amber-600" 
                isLoading={loading}
                disabled={!form.pacienteId || !form.medicamento || !form.prescripcionAnteriorId}
...
export default PrescripcionAlergiaPage;
